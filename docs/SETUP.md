...
Dependency installs are **opt-in, always**: only `/otel-setup` installs
anything, and it activates tracing live in the same session — verified,
no restart needed. If a code-puppy update or uvx cache prune rebuilds
the environment without the deps, the startup banner says so and one
`/otel-setup` re-onboards that environment. To skip even that, bake the
deps into your launch command:
...
| Startup banner: "...opentelemetry-sdk ... aren't installed" | Deps missing from this environment (fresh env, code-puppy update, uvx cache prune) | `/otel-setup` — installs and activates live, no restart |
| `/otel-setup` says "deps installed, but this running process couldn't hot-load them" | Rare: Python's import state couldn't absorb the freshly installed packages (healthy on disk) | Restart once; tracing activates automatically at startup |
